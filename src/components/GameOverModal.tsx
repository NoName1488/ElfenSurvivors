...
import { Trophy, Skull, RotateCcw, Dna, Swords, Flame, Clock, Sparkles, ArrowRight, Unlock } from 'lucide-react';
  ...
  return (
    <div id="game-over-modal" className="fixed inset-0 bg-[#050505]/95 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in zoom-in-95 duration-200 select-none">
      <div className={`max-w-lg w-full glass-panel rounded-2xl p-6 md:p-8 shadow-2xl flex flex-col items-center gap-6 border ${
        ...
        {newlyUnlockedCharacter && (
          <div className="w-full glass-panel-crimson p-4 rounded-xl border border-amber-400/80 bg-gradient-to-r from-amber-950/40 via-red-950/40 to-amber-950/40 flex items-center gap-4 shadow-[0_0_30px_rgba(245,158,11,0.3)] animate-pulse">
            <div
              className="w-12 h-12 rounded-lg flex items-center justify-center font-cinzel font-black text-xl text-white shadow-xl border border-amber-400"
            ...
            <div className="flex-1">
              <div className="text-[10px] font-mono uppercase tracking-[0.2em] text-amber-400 font-bold flex items-center gap-1.5">
                ...
                <span>{isRu ? 'РАЗБЛОКИРОВАН НОВЫЙ БОЕВОЙ СУБЪЕКТ!' : 'NEW COMBAT SUBJECT UNLOCKED!'}</span>
              </div>
