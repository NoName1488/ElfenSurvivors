    ...
    <div id="game-over-modal" className="fixed inset-0 bg-[#050505]/95 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in zoom-in-95 duration-200 select-none">
      <div className={`max-w-lg w-full max-h-[95vh] overflow-y-auto glass-panel rounded-2xl p-6 md:p-8 shadow-2xl flex flex-col items-center gap-6 border ${
        ...
        {engine.trialUnlocks.length > 0 && <p className="text-amber-300 text-base" role="status">
          {isRu ? 'Открыты персонажи: ' : 'Characters unlocked: '}{engine.trialUnlocks.map(c => characterName(c, isRu)).join(', ')}
        </p>}
        ...
        {newlyUnlockedCharacter && (
          <div className={`w-full glass-panel-crimson p-4 rounded-xl border flex items-center gap-4 animate-pulse ${
            ...
            >
              {characterName(newlyUnlockedCharacter, isRu)[0]}
            </div>
            <div className="flex-1">
              <div className={`text-xs font-mono uppercase tracking-[0.2em] font-bold flex items-center gap-1.5 ${
        ...
        {s.activeSynergies.length > 0 && (
          <div className="w-full glass-panel p-3 rounded-xl border-amber-500/30 bg-amber-950/20 flex flex-col gap-1.5">
            <div className="text-xs font-mono uppercase tracking-[0.2em] text-amber-400 font-bold flex items-center gap-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {s.activeSynergies.map((syn) => (
                <span
                  key={syn.id}
                  className="px-2 py-0.5 rounded bg-amber-500/20 border border-amber-500/40 text-xs font-mono text-amber-200 font-bold flex items-center gap-1"
                >
                  <span>{syn.icon}</span>
        ...
        <div className="w-full glass-panel p-3.5 rounded-xl border-white/10 flex flex-col gap-2">
          <div className="text-xs font-mono uppercase tracking-[0.2em] text-red-500 font-bold flex items-center gap-1.5">
          ...
          <div className="flex flex-wrap gap-1.5">
            {s.weapons.map((w, idx) => (
              <span
                key={idx}
                className="px-2.5 py-1 rounded border border-white/10 bg-white/5 text-xs font-mono text-gray-300"
              >
