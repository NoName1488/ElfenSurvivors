  ...
  if (!treeData) {
    return (
      <div className="p-8 text-center text-gray-400 font-mono">
        Мутационное древо для данного субъекта не обнаружено.
      </div>
    );
  }
  ...
  return (
    <div className={`flex flex-col h-full w-full ${isModal ? 'bg-[#050505] p-4 md:p-6' : ''}`}>
      ...
      <div className="flex flex-wrap items-center justify-between pb-4 mb-4 border-b border-red-950/40 gap-4">
        <div>
          <div className="flex items-center gap-2 text-[11px] font-mono tracking-[0.2em] text-red-400 font-bold">
            <Cpu className="w-4 h-4 text-red-500 animate-pulse" />
            <span>
              {isCyborg ? 'КИБЕРНЕТИЧЕСКАЯ СПЕЦИАЛИЗАЦИЯ' : 'ДРЕВО ПСИХИЧЕСКИХ МУТАЦИЙ ВЕКТОРОВ'}
            </span>
          </div>
          ...
          <p className="text-xs text-gray-400 font-mono mt-0.5 max-w-2xl">{treeData.loreIntro}</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2.5 px-4 py-2 rounded-xl bg-red-950/40 border border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.2)]">
            ...
            <div>
              <div className="text-[9px] font-mono uppercase text-gray-400 font-bold">Очки мутации</div>
          ...
          <button
            ...
            title="Сбросить все распределенные очки мутации"
          >
          ...
          >
            ...
            <div className="flex items-center justify-between pb-3 mb-3 border-b border-neutral-800/60">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="font-cinzel font-bold text-sm text-white">{branch.name}</div>
                  <div className="text-[10px] text-red-400 font-mono">{branch.description}</div>
                </div>
              </div>
            </div>
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5">
                          ...
                          >
                            {node.tier === 3 ? '★ ВЕРШИНА ТИР-3' : `ТИР ${node.tier}`}
                          </span>
                        </div>

                        <div>
                          {isUnlocked ? (
                            <span className="flex items-center gap-1 text-[10px] font-mono font-bold text-emerald-400 bg-emerald-950/40 px-2 py-0.5 rounded border border-emerald-500/30">
                              <CheckCircle2 className="w-3 h-3" /> АКТИВНО
                            </span>
                          ) : canUnlock ? (
                            <span className="flex items-center gap-1 text-[10px] font-mono font-bold text-amber-300 bg-amber-950/40 px-2 py-0.5 rounded border border-amber-500/40">
                              ДОСТУПНО ({node.cost} ОЧК.)
                            </span>
                          ) : (
                      ...
                      {/* Special Perk Tag */}
                      {node.specialPerkId && (
                        <div className="flex items-center gap-1.5 text-[10px] font-mono text-purple-300 bg-purple-950/40 p-1.5 rounded border border-purple-800/40 mt-0.5">
                          <Sparkles className="w-3 h-3 text-purple-400 shrink-0" />
                          <span>Уникальная механика: {node.russianName || node.name}</span>
                        </div>
                      )}
                      ...
                      {canUnlock && (
                        ...
                        >
                          ...
                          <span>РАЗБЛОКИРОВАТЬ ЭВОЛЮЦИЮ</span>
                        </button>
                      )}
                    </div>
                  </React.Fragment>
      ...
      <div className="mt-2 pt-3 border-t border-neutral-800/60 flex flex-wrap items-center justify-between gap-4 text-xs font-mono text-gray-400">
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1.5 text-red-400">
            ...
            <span>Активных мутаций: {mutationState.unlockedNodeIds.length}</span>
          </span>
          <span className="text-gray-600">|</span>
          <span>Векторов: {engine.state.stats.vectorCount}</span>
          <span className="text-gray-600">|</span>
          <span>Дальность векторов: {engine.state.stats.vectorReach}px</span>
          <span className="text-gray-600">|</span>
          <span>Скорость векторов: +{engine.state.stats.attackSpeed}%</span>
        </div>

        <div className="text-[11px] text-gray-500 flex items-center gap-1.5">
          <Info className="w-3.5 h-3.5" />
          <span>Каждый уровень дает +1 очко психической мутации</span>
        </div>
      </div>
    </div>
  );
